    ...
    if 'links' in dashboard:
        for link in dashboard['links']:
            if 'title' in link and link['title'] == 'Report':
                ...
                link['type'] = "link"
                link['url'] = "{0}api/report/{1}?apitoken={2}".format(
                    dest['report_url'], dest['titles'][type_].lower(), dest['apikey'])
...
if __name__ == '__main__':
    for type_ in src['dashboards']:
        ...
        for dest in dests:
            ...
            if 'user' in dest:
                ret = import_dashboard_via_user_pass(
                    dest['url'], dest['user'], dest['password'], dashboard)
            else:
